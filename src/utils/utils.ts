import { domainmodels, EnumProperty, IModel, microflows, projects, security } from "mendixmodelsdk";
import { MendixPlatformClient } from "mendixplatformsdk";
import input from '../input.json';
import { PrimitiveType } from "../types/AttributeType";

export const getOrCreateDomainModel = async (model : IModel): Promise<domainmodels.DomainModel> =>
{
...
{
    const existingDomainModelInterface = model.allDomainModels().filter(dm => dm.containerAsModule.name === input.Module)[0];
...
const createDomainModelInterface = (model: IModel) : domainmodels.IDomainModel =>
{
...
export const getOrCreateFolder = (location: projects.IFolderBase, name: string) => {
...
const createFolder = (location: projects.IFolderBase, name: string) => {
    ...
    export const getOrCreateEntity = (domainModel: domainmodels.DomainModel, entityName: string, x : number, y : number) => {
    ...
    const createEntity = (domainModel: domainmodels.DomainModel, entityName: string, x : number, y : number) => {
        const newEntity = domainmodels.Entity.createIn(domainModel);
        newEntity.name = entityName;
        newEntity.location = { x: x, y: y};
        return newEntity;
    };
    export const getOrCreateAttribute = (Entity : domainmodels.Entity, attributeName : string, attributeType ?: PrimitiveType, length ?:number) => {
    ...
    const createAttribute = (Entity : domainmodels.Entity, attributeName : string, attributeType ?: PrimitiveType, length ?: number) => {
        ...
        switch (type) {
            ...
            case PrimitiveType.BOOLEAN :
                domainmodels.BooleanAttributeType.createInAttributeUnderType(NewAttribute);
                break;
            case PrimitiveType.DATE || PrimitiveType.DATE_TIME :
                domainmodels.DateTimeAttributeType.createInAttributeUnderType(NewAttribute);
                break;
            case PrimitiveType.DECIMAL :
                domainmodels.DecimalAttributeType.createInAttributeUnderType(NewAttribute);
                break;
            case PrimitiveType.INTEGER  :
                domainmodels.IntegerAttributeType.createInAttributeUnderType(NewAttribute);
                break;
            case PrimitiveType.LONG  :
                domainmodels.LongAttributeType.createInAttributeUnderType(NewAttribute);
                break;
            case PrimitiveType.STRING  :
                const Attr = domainmodels.StringAttributeType.createInAttributeUnderType(NewAttribute);
                if (length)
                    Attr.length = length;
...
export const createMicroflow = (location: projects.IFolderBase, name: string) => {
    const microflow = microflows.Microflow.createIn(location);
    microflow.name = name;
    return microflow;
};

export const createStartEvent = (microflow: microflows.Microflow): microflows.StartEvent => {
    const start = microflows.StartEvent.createIn(microflow.objectCollection);
    start.relativeMiddlePoint = { x: 0, y: 100 };
    return start;
};

export const createEndEvent = (microflow: microflows.Microflow, x: number): microflows.EndEvent => {
    const end = microflows.EndEvent.createIn(microflow.objectCollection);
    end.relativeMiddlePoint = { x: x, y: 100 };
    return end;
};

const createMicroflowAction = (microflow: microflows.Microflow, x: number, widthFactor: number): microflows.ActionActivity => {
    const actionActivity = microflows.ActionActivity.createIn(microflow.objectCollection);
    actionActivity.relativeMiddlePoint = { x: x, y: 100 };
    actionActivity.size.width = actionActivity.size.width * widthFactor;
    return actionActivity;
};

export const createCreateAction = (microflow: microflows.Microflow, entity: domainmodels.Entity): microflows.ActionActivity => {
...
export function connectMicroflowActions(microflow: microflows.Microflow, start: microflows.MicroflowObject, end: microflows.MicroflowObject, connectionType: ConnectionType): microflows.SequenceFlow {
